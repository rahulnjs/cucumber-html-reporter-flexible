...
var generateReport = function (options) {
    var featureOutput;
    if (options.jsonFile) {
        featureOutput = jsonFile.readFileSync(options.jsonFile);
    } else {
        featureOutput = options.json;
    }
    ...
    try {
        if (options.customScripts && options.customScripts.length > 0) {
            for (let cs of options.customScripts) {
                const customScriptsFile = searchFileUp(cs);
                _cScripts += '\n\n' + fs.readFileSync(customScriptsFile, 'utf-8');
            }
        }

        if (options.customStyles && options.customStyles.length > 0) {
            for (let cs of options.customStyles) {
                const customStylesFile = searchFileUp(cs);
                _cStyles += '\n\n' + fs.readFileSync(customStylesFile, 'utf-8');
            }
        }
    } catch (e) { }


    const html = _.template(readFile('index.html'))({
        suite: suite,
...
function generate(options, callback) {
    ...
    function isValidJsonFile() {
        ...
        if (options.jsonFile && !isAFile(options.jsonFile)) {
